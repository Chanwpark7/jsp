...
@WebServlet("/updown/multiUploadProc.do")
@MultipartConfig(
	maxFileSize = 1024 * 1024 * 1,
	maxRequestSize = 1024 * 1024 * 10
)
public class MultiUploadeController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    	...
    	try {
    		ArrayList<String> orgNames = FileUtil.multiFileUp(req, saveDir);//파일 업로드 처리
			//파일의 가상이름 생성 및 DB Insert
    		for(int i = 0; i<orgNames.size();i++) {
    			String saveName = FileUtil.renameFile(saveDir, orgNames.get(i));
    			
    			//DAO 호출 해서 insert 메소드 호출
    			insertMyFile(req, orgNames.get(i), saveName);
    		}
